...
func (ots *OracleTestSuite) TestPrices() {
	...
	ots.Require().Equal(sdk.MustNewDecFromStr("3.710916056220858266"), prices["NEBULA"])
	...
	prices = ots.oracle.GetPrices()
	ots.Require().Len(prices, 4)
	ots.Require().Equal(sdk.MustNewDecFromStr("3.70"), prices["NEBULA"])
	ots.Require().Equal(sdk.MustNewDecFromStr("3.717"), prices["XBT"])
	ots.Require().Equal(sdk.MustNewDecFromStr("1"), prices["USDC"])
	ots.Require().Equal(sdk.MustNewDecFromStr("1"), prices["USDT"])
	...
	prices = ots.oracle.GetPrices()
	ots.Require().Len(prices, 4)
	ots.Require().Equal(sdk.MustNewDecFromStr("3.71"), prices["NEBULA"])
	ots.Require().Equal(sdk.MustNewDecFromStr("3.717"), prices["XBT"])
	ots.Require().Equal(sdk.MustNewDecFromStr("1"), prices["USDC"])
	ots.Require().Equal(sdk.MustNewDecFromStr("1"), prices["USDT"])
}
...
func TestGenerateExchangeRatesString(t *testing.T) {
	testCases := map[string]struct {
		...
		"single denom": {
			input: map[string]sdk.Dec{
				"NEBULA": sdk.MustNewDecFromStr("3.72"),
			},
			expected: "NEBULA:3.720000000000000000",
		},
		"multi denom": {
			input: map[string]sdk.Dec{
				"NEBULA": sdk.MustNewDecFromStr("3.72"),
			...
			expected: "ATOM:40.130000000000000000,OSMO:8.690000000000000000,NEBULA:3.720000000000000000",
		},
	}
